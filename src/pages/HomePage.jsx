    ...
    <div style={{ background: T.bg, color: T.w, minHeight: '100vh', width: '100%', overflowX: 'hidden' }}>
            ...
            {[
              ...
              <div key={s.n}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: T.gnGl, border: `1px solid ${T.gnB}`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 36 }}>
                  <span className="mono" style={{ fontSize: 11, color: T.gn, fontWeight: 700 }}>
                    {s.n}
                  </span>
                </div>
      ...
      <section style={{ padding: '80px 0', background: T.bg }}>
        <div className="se-shell">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 14, marginBottom: 24 }}>
            ...
            <button className="bs" onClick={() => setPage('artist')} style={{ padding: '10px 18px', fontSize: 13.5 }}>
              View All →
            </button>
          </div>
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginBottom: 22 }}>
            {GENRES.map((g) => (
              <button key={g} className={`chip ${genre === g ? 'csel' : 'cb'}`} onClick={() => setGenre(g)} style={{ cursor: 'pointer' }}>
                {g}
              </button>
            ))}
          </div>
          ...
          {displayCurators.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '42px 18px', border: `1px dashed ${T.b1}`, borderRadius: 16, color: T.g300, marginTop: 10 }}>
              ...
              <div style={{ fontSize: 13, color: T.g400, maxWidth: 520, margin: '0 auto' }}>In live mode, featured curators will populate from your connected backend marketplace.</div>
            </div>
          ) : null}
          <div style={{ textAlign: 'center', marginTop: 24 }}>
            <button className="bp" onClick={startCampaign} style={{ padding: '13px 30px', fontSize: 15 }}>
              Start Your Campaign <span className="arr">→</span>
            </button>
          </div>
        </div>
      </section>
        ...
        <div className="se-shell" style={{ maxWidth: 860 }}>
          ...
          <div style={{ background: `linear-gradient(145deg,${T.card},#0d0d10)`, border: `1px solid ${T.b0}`, borderRadius: 18, padding: '20px 18px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ color: T.g200, fontSize: 13.5, lineHeight: 1.6 }}>See plan tiers and buy credits in one place — or open billing from your dashboard after sign-in.</div>
      ...
      <section style={{ padding: '80px 0', background: T.bg }}>
        <div className="se-shell">
          ...
          <p style={{ color: T.g200, fontSize: 14.5, maxWidth: 560, lineHeight: 1.7 }}>Once you start submitting, StreamEngine will show your submission history and curator responses here and in your dashboard.</p>
        </div>
      </section>
        ...
        <div className="se-shell" style={{ maxWidth: 680 }}>
          ...
          {FAQS_DATA.map((f, i) => (
            <div key={i} style={{ borderBottom: '1px solid rgba(255,255,255,.05)' }}>
              <button
                style={{ width: '100%', textAlign: 'left', background: 'none', padding: '18px 0', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: T.w, fontSize: 15, fontWeight: 600, transition: 'color .15s' }}
                onClick={() => setOpenFaq(openFaq === i ? null : i)}
                onMouseEnter={(e) => {
                  e.currentTarget.style.color = T.gn
                }}
                onMouseLeave={(e) => {
              ...
              {openFaq === i ? <div style={{ fontSize: 14, color: T.g200, lineHeight: 1.75, paddingBottom: 18, animation: 'fadeUp .2s ease' }}>{f.a}</div> : null}
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '90px 0', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="se-shell" style={{ maxWidth: 580, position: 'relative' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px,6vw,56px)', fontWeight: 900, letterSpacing: '-.04em', lineHeight: 1.05, marginBottom: 18 }}>
            Ready to grow on
            <br />
            <span style={{ color: T.gn }}>Spotify?</span>
          </h2>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', maxWidth: 360, margin: '0 auto' }}>
            <button className="bp" onClick={startCampaign} style={{ flex: 1, padding: '15px 20px', fontSize: 16, minWidth: 140 }}>
              Get Started Free <span className="arr">→</span>
            </button>
            <button type="button" className="bs" onClick={goPricingPage} style={{ flex: 1, padding: '15px 20px', fontSize: 16, minWidth: 110 }}>
              Pricing
            </button>
          </div>
            ...
            <div className="se-footer-links">
              {[
                ...
                <button
                  ...
                  onMouseLeave={(e) => {
                    e.currentTarget.style.color = T.g300
                  }}
                >
                  {x.l}
                </button>
